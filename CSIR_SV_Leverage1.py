...
def particle_filter(observations, initial_particles, likelihood_func, transition, N, seed=1234):
    ...
    for i in range(T - 1):
        ...
        likelihood, normalized_weights = importance_ratio(
            likelihood_func, observations[i + 1], new_particles)
...
def transition_sample(x, y):
    return mu * (1 - phi) + phi * x + np.sqrt(sigma_eta_square) * rho * y * np.exp(-x / 2) + np.sqrt(sigma_eta_square * (1 - rho**2)) * np.random.randn(1)
...
mus = [i * 0.02 for i in range(20, 31)]
