...
# beta0s = [i * 0.002 for i in range(10)]
sigmas = [i * 0.02 for i in range(1,11)]

cumulated_loglikelihoods = np.zeros((50, len(sigmas)))
estimations = np.zeros(50)
for seed in range(50):
    print('sigma iteration {}'.format(seed))
    loglikelihoods = np.zeros(len(sigmas))
    ...
    estimations[seed] = sigmas[np.argmax(loglikelihoods)]
...
# cumulated_loglikelihoods = np.zeros((50, len(beta0s)))
# estimations = np.zeros(50)
# for seed in range(50):
#     print('beta0 iteration {}'.format(seed))
#     loglikelihoods = np.zeros(len(beta0s))
#     for i in range(len(beta0s)):
#         beta0 = beta0s[i]
#         likelihoods = particle_filter(observations=observations, initial_particles=initial_particles,
#                                       likelihood_func=likelihood_function, transition=transition_sample, N=N, seed=seed)
#         loglikelihood = sum(np.log(likelihoods))
#         loglikelihoods[i] = loglikelihood
#     print(loglikelihoods)
#     estimations[seed] = beta0s[np.argmax(loglikelihoods)]
#     cumulated_loglikelihoods[seed] = loglikelihoods
# print(estimations)
# means = np.mean(cumulated_loglikelihoods, axis=0)
# variances = np.var(cumulated_loglikelihoods, axis=0)
